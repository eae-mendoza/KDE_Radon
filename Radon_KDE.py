...
def f_beta_estimates(xy_sample, dist_range, d, h, C_rad, probabilities):
    f_estimates = np.array(
        [[fbetahat(xy_sample, dist_range[i][j], d, h, C_rad, probabilities) for i in range(0, len(dist_range))] for j in
         range(0, len(dist_range))])
    return f_estimates
...
def fbetahat(sample, t, d, h, C_radtf, probabilities, stilde_range, hilbert_array):
    ...
    hilb_phi = np.array(
        [hilbert_interpolator(s_tilde(i[2], i[0:2], t, h), stilde_range, hilbert_array) for i in sample])
